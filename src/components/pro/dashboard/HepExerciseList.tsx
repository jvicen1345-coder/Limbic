...
const EMPTY_ROW: HepTemplateExercise = { name: "", sets: "", reps: "", weight: "", frequency: "", hold: "", notes: "", imageUrl: "", videoUrl: "" };
  ...
  const emit = (next: DraftRow[]) => {
    ...
    onChange(
      next.map((r) => ({
        ...
        reps: r.reps,
        weight: r.weight,
        // Neither has a dedicated input in the compact editor below — both just preserved
        // as-is so a row loaded from a HepBuilder-authored template (which does collect
        // them) doesn't silently lose either on the next edit-and-resave here.
