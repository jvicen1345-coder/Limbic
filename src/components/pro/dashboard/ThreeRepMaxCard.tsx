...
import { LIFTS, SEXES, STRENGTH_LEVEL_LABELS, type Lift, type Sex, type StrengthLevel } from "@/lib/three-rep-max-standards";
    ...
    <div className="trm-lift-row">
      <div>
        ...
        <div className="trm-lift-meta">
          {Math.round(entry.weightLbs)} lbs × 3 · est. 1RM {Math.round(entry.oneRepMaxLbs)} lbs · {new Date(entry.testedAt).toLocaleDateString()}
        </div>
      </div>
 ...
 *  Bench Press, Deadlift — each classified against bodyweight-relative strength standards
 *  (see lib/three-rep-max-standards.ts) so a clinician has quick general-population context
 *  to discuss with the patient, not just a raw number. */
export function ThreeRepMaxCard({ patientId, patientCode }: { patientId: string; patientCode: string }) {
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      const result = await createThreeRepMaxTest(patientId, lift, weight, bodyweight, sex);
      if ("error" in result) {
