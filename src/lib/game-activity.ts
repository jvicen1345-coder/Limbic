...
export async function recordGameActivity(userId: string, dateKey: string, timeZone: string) {
  const user = await prisma.user.findUnique({
    where: { id: userId },
    select: { lastGamesActivityAt: true, gamesStreakDays: true },
  });
  if (!user) return;
  const gamesStreakDays = nextStreak(user.lastGamesActivityAt, user.gamesStreakDays, timeZone, dateKey);
  await Promise.all([
