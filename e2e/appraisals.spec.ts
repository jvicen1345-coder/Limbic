 ...
 *
 * Both assertions live in ONE test on purpose, against one account and one database
 * session. Split across two, this file added two more sign-ups and two more libSQL write
 * connections to a `fullyParallel` suite that already contends for a single SQLite write
 * lock — and the cost landed on copyright-moderation.spec.ts, which writes the same way:
...
/** Inserts one published and one draft appraisal for `email`, over a single connection held
 *  for as short a time as possible — see the note at the top of this file on why this is one
...
async function insertAppraisals(email: string, titles: { published: string; draft: string }) {
  return withDb(async (db) => {
    ...
    const ids = { published: `e2e-appraisal-pub-${stamp}`, draft: `e2e-appraisal-draft-${stamp}` };
    ...
    for (const status of ["published", "draft"] as const) {
      await db.execute({
        ...
        args: [
          ...
          authorId,
          status,
          JSON.stringify(appraisalInput(titles[status])),
...
test.describe("appraisals", () => {
  test("a published appraisal carries the computed findings; a draft is not readable", async ({ page }) => {
    ...
    const ids = await insertAppraisals(email, { published: title, draft: `Draft appraisal ${Date.now()}` });

    await page.goto(`/article/appraisal-${ids.published}`);
