...
| Three fonts, one accent, semantic colour only for a real level dimension | `src/styles/tokens.css` and `src/styles/playbooks.css` | Already set by the app's type and colour system. The only semantic dimension in the content is irritability: pills `[[pill:h\|…]]`, `m`, `l` and the `PlaybookCard.tone` of the same letters. Don't invent a second colour axis. |
